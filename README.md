...
- **Want a classic without hunting for the file?** `npm run books:find -- "moby dick"` searches [Project Gutenberg](https://www.gutenberg.org) (75k+ public-domain books); `npm run books:find -- --get <ID>` downloads the epub into `books/`, then `npm run books:sync` turns it into audio. The Gutenberg license header/footer is stripped automatically. Public-domain works only — in-copyright books are not downloaded
...
- **想听名著但懒得找文件?** `npm run books:find -- "moby dick"` 搜 [Project Gutenberg](https://www.gutenberg.org)(7.5 万+ 公版书),`npm run books:find -- --get <ID>` 把 epub 下到 `books/`,再 `npm run books:sync` 转音频;Gutenberg 的许可声明会自动剥掉。中文书用罗马字/英文名搜(如 `hong lou meng`)。**仅限版权已过期的公版书**,在版权期的书不走这里
